...
If next has a snake or ladder, you must move to the destination of that snake or ladder. Otherwise, you move 
to next.
The game ends when you reach the square n^2.
A board square on row r and column c has a snake or ladder if board[r][c] != -1. The destination of that snake 
or ladder is board[r][c]. Squares 1 and n^2 are not the starting points of any snake or ladder.
...
def snakesAndLadders(board):
    ...
    while q:
        ...
        for i in range(1, 7):
            next_cell = cell + i
            r, c = get_cell_position(next_cell)
            if board[r][c] != -1:
                next_cell = board[r][c]
            if next_cell == n * n:
                return moves + 1
...
print(snakesAndLadders(board)) # Expected: 4

board = [[-1,-1],
         [-1, 3]]
print(snakesAndLadders(board)) # Expected: 1
