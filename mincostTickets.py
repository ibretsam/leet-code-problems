...
def mincostTickets(days, costs):
    ...
    def dfs(index):
        ...
        if index in cache:
            return cache[index]
        one_day = costs[0] + dfs(index + 1)
        seven_days = costs[1] + dfs(bisect_left(days, days[index] + 7))
        thirty_days = costs[2] + dfs(bisect_left(days, days[index] + 30))
        cache[index] = min(one_day, seven_days, thirty_days)
        return cache[index]
    return dfs(0)
