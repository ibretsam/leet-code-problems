...
are on index i on the current row, you may move to either index i or index i + 1 on the next 
row.
...
def minimumTotal(triangle):
    def dfs(level, i, memo = {}):
        if (level, i) in memo:
            return memo[(level, i)]
        if level == len(triangle) - 1:
            return triangle[level][i]
        memo[(level, i)] = triangle[level][i] + min(dfs(level + 1, i, memo), dfs(level + 1, i + 1, memo))
        return memo[(level, i)]
    return dfs(0, 0)
